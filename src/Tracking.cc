...
void Tracking::StereoInitialization()
{
    ...
    {
        if ((mSensor == System::IMU_STEREO) && (mpAtlas->KeyFramesInMap() > 2))
        {
...
{
    ofstream outputfile("diff.txt");
    static ofstream outputfile2("diff2.txt");
    outputfile2<<mnLastKeyFrameId<<"\t";
    outputfile2<<setprecision(20)<<mpLastKeyFrame->mTimeStamp<<"\n";
    vector<KeyFrame*> vKF5 = mpAtlas->GetAllKeyFrames();
    //int sz = vKF5.size();
    for (int it = 0;it < vKF5.size();++it) {
        outputfile<<vKF5.at(it)->mnId<<"\t";
        outputfile<<setprecision(20)<<vKF5.at(it)->mTimeStamp<<"\n";
    }
    //c-->ut<<mCurrentFrame.mnId<<endl;
    ...
    if (mpLocalMapper->mbBadImu2)
    {
        mpLocalMapper->mbBadImu2 = false;
        return false;
    }

    if ((cv::norm(mCurrentFrame.mOw - mCurrentFrame.mpLastKeyFrame->GetCameraCenter()) < 0.02) && (nKFs > 2))
        return false;
    ...
    */
    ...
    cv::Mat deltaavgA = mCurrentFrame.mpImuPreintegratedFrame->avgA-mLastFrame.mpImuPreintegratedFrame->avgA;
    float nDelAvg = sqrt(pow(deltaavgA.at<float>(0),2) + pow(deltaavgA.at<float>(1),2) + pow(deltaavgA.at<float>(2),2));
    bool accCheck = (nDelAvg > 1) && ((mCurrentFrame.mnId - mnLastKeyFrameId) > 1);
    ...
    bool Vc = VisibilityMatrixCondition();
    bool c3 = (bNeedToInsertClose && mnMatchesInliers>15 && bLocalMappingIdle);

    //if(((c1a||c1b||c1c) && c2)||c3 ||c4)
    //if(Vc || c3 || accCheck || (nDelAvg > 2))
    //if(((Vc || (bNeedToInsertClose && mnMatchesInliers>15)) && bLocalMappingIdle) || accCheck)
    if(Vc || c3 || accCheck || c1 || c2)
    {
...
bool Tracking::VisibilityMatrixCondition()
{
    /*const int nKFs = mpAtlas->KeyFramesInMap();
    float distth = 0;
    if (nKFs > 10)
    ...
    bool distanceCheck = cv::norm(mCurrentFrame.mOw - mCurrentFrame.mpLastKeyFrame->GetCameraCenter()) > distth;


    if(((mCurrentFrame.mnId - mnLastKeyFrameId) >= 6))
    {
        if (distanceCheck)
        {
            return true;
    ...
    vector<int> IndexInmvpMapPoint;
    cv::Mat decm;
    cv::Mat mCurrentFrameDescriptors;
    for(int i=0; i<mCurrentFrame.N; ++i)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            if(i > 0)
            {
                auto it = find(points.begin(),points.end() , cv::Point3f(mCurrentFrame.mvpMapPoints[i]->GetWorldPos()));

                int findedindex = it - points.begin();

                if(findedindex != points.size())
                    continue;
            }
            points.push_back(cv::Point3f(mCurrentFrame.mvpMapPoints[i]->GetWorldPos()));
            decm = mCurrentFrame.mvpMapPoints[i]->GetDescriptor();
            mCurrentFrameDescriptors.push_back(decm);
            IndexInmvpMapPoint.push_back(i);
        }
    }
    ...
    for(int i = 0; i < points.size(); ++i)
    {
        ...
        std::vector<float> AllInd;
        for (int j = 0; j < points.size(); ++j)
        {
            float dP1P2 = sqrt(pow((points[i].x - points[j].x),2) + pow((points[i].y - points[j].y),2) + pow((points[i].z - points[j].z),2));

            if(i == j)
                continue;

            AllDists.push_back(dP1P2);

            AllInd.push_back(j);
        }

        std::vector<cv::Point3f> NearestVec;
        for (int k = 0; k < 5; ++k)
        {
            float minimumDist = *min_element(AllDists.begin(), AllDists.end());

            auto it2 = find(AllDists.begin(),AllDists.end() , minimumDist);

            int index = it2 - AllDists.begin();

            NearestVec.push_back(cv::Point3f((points[AllInd[index]].x - points[i].x), (points[AllInd[index]].y - points[i].y), (points[AllInd[index]].z - points[i].z)));

            AllDists.erase(AllDists.begin() + index);
            AllInd.erase(AllInd.begin() + index);
        }
        ...
        for (int k = 0; k < 4; ++k)
        {
            for(int t = k + 1; t < 5; ++t)
            {
                sx += NearestVec[k].y * NearestVec[t].z - NearestVec[k].z * NearestVec[t].y;
                sy += NearestVec[k].z * NearestVec[t].x - NearestVec[k].x * NearestVec[t].z;
                sz += NearestVec[k].x * NearestVec[t].y - NearestVec[k].y * NearestVec[t].x;
            }
        }
        float NormNearestVec = sqrt(pow(sx,2) + pow(sy,2) + pow(sz,2));
        sx /= NormNearestVec;
        sy /= NormNearestVec;
        sz /= NormNearestVec;

        norms.push_back(cv::Point3f(sx,sy,sz));

    }

    cv::Mat PointToCameraVector;
    cv::Mat mCurrentFrameVisibilityMatrix = cv::Mat::zeros(points.size(),1,CV_32F);
    float angle;
    float coneAngle = 10;
    float geometryAngle = 30;
    vector<float> Angles;
    for (int i = 0; i < points.size(); ++i)
    {
        PointToCameraVector = mCurrentFrame.mOw - mCurrentFrame.mvpMapPoints[IndexInmvpMapPoint[i]]->GetWorldPos();
        angle = acos((norms[i].x*PointToCameraVector.at<float>(0) + norms[i].y*PointToCameraVector.at<float>(1) + abs(norms[i].z)*PointToCameraVector.at<float>(2))/(cv::norm(PointToCameraVector)))*180/3.14159265358979;
        Angles.push_back(angle);

        float* RowInd = mCurrentFrameVisibilityMatrix.ptr<float>(i);
        if(angle <= coneAngle)
        {
            *RowInd = 1;
        }
        else if(angle <= 2*coneAngle && angle > coneAngle)
        {
            *RowInd = 2;
        }
        else if(angle <= 3*coneAngle && angle > 2*coneAngle)
        {
            *RowInd = 3;
        }
        else if(angle > 3*coneAngle)
        {
            *RowInd = 4;
        }
    }

    cv::Mat sRcw = mCurrentFrame.mTcw.rowRange(0,3).colRange(0,3);
    cv::Mat tcw = mCurrentFrame.mTcw.rowRange(0,3).col(3);

    static cv::Mat mnLastKeyFrameDescriptors;
    static cv::Mat mnLastKeyFrameVisibilityMatrix;
    static std::vector<cv::Point3f> mnLastKeyFrameWorldPoints;
    static std::vector<MapPoint*> LastKFmvpMapPoints;
    static vector<int> IndexInmvpMapPointLastKF;
    static vector<float> LastKeyFrameAngles;
    static float matchr = 0;
    static float diffr = 0;
    static float CFr = 0;

    if(mCurrentFrame.mnId == mnLastKeyFrameId)
    {
        mnLastKeyFrameDescriptors = mCurrentFrameDescriptors;
        mnLastKeyFrameVisibilityMatrix = mCurrentFrameVisibilityMatrix;
        mnLastKeyFrameWorldPoints = points;
        LastKFmvpMapPoints = mCurrentFrame.mvpMapPoints;
        IndexInmvpMapPointLastKF = IndexInmvpMapPoint;
        LastKeyFrameAngles = Angles;
    }
    else
    {
        /*float goBackForthCheck = cv::norm(mCurrentFrame.mOw - mCurrentFrame.mpLastKeyFrame->GetCameraCenter()) + cv::norm(mCurrentFrame.mpLastKeyFrame->GetCameraCenter() - mCurrentFrame.mpLastKeyFrame->mPrevKF->GetCameraCenter());
        if((goBackForthCheck < distth))
            return false;

        if((cv::norm(mCurrentFrame.mOw - mCurrentFrame.mpLastKeyFrame->GetCameraCenter()) < distth))
            return false;*/

        int nDifferents = 0;
        int nBigAngleChange = 0;
        float meanAngles = 0;
        int nMatchesLastKFandCurrentF = 0;
        cv::Mat gridMatrix = cv::Mat::zeros(3, 3, CV_32F);
        float RangeX = mCurrentFrame.mnMaxX - mCurrentFrame.mnMinX;
        float RangeY = mCurrentFrame.mnMaxY - mCurrentFrame.mnMinY;
        float StepX = RangeX / 3;
        float StepY = RangeY / 3;

        for(int i = 0 ; i < mnLastKeyFrameDescriptors.rows ; ++i)
        {
            // 3D in Current camera coordinates
            cv::Mat x3Dc = sRcw*(cv::Mat)mnLastKeyFrameWorldPoints[i]+tcw;

            // Check positive depth
            if(x3Dc.at<float>(2)<0.0f)
                continue;

            const cv::Point2f uv = mCurrentFrame.mpCamera->project(x3Dc);

            if(uv.x<mCurrentFrame.mnMinX || uv.x>mCurrentFrame.mnMaxX)
                continue;
            if(uv.y<mCurrentFrame.mnMinY || uv.y>mCurrentFrame.mnMaxY)
                continue;

            vector<size_t> vIndices2 = mCurrentFrame.GetFeaturesInArea(uv.x, uv.y,10);
            ...
            {
                if(!mCurrentFrame.mvpMapPoints[vIndices2[j]])
                    continue;

                dist.push_back(ORBmatcher::DescriptorDistance(mCurrentFrame.mvpMapPoints[vIndices2[j]]->GetDescriptor() , mnLastKeyFrameDescriptors.row(i)));

                auto it = find(IndexInmvpMapPoint.begin(), IndexInmvpMapPoint.end(), vIndices2[j]);

                index.push_back(it - IndexInmvpMapPoint.begin());
            }

            if(!dist.empty())
            {
                int minimumDist = *min_element(dist.begin(), dist.end());

                if(minimumDist < 30)
                {
                    ++nMatchesLastKFandCurrentF;

                    auto it2 = find(dist.begin(),dist.end() , minimumDist);

                    int index2 = it2 - dist.begin();

                    if((mCurrentFrameVisibilityMatrix.at<float>(index[index2]) != mnLastKeyFrameVisibilityMatrix.at<float>(i)))
                    {
                        ++nDifferents;

                        if(abs(Angles[index[index2]] - LastKeyFrameAngles[i]) > geometryAngle)
                        {
                            cv::Mat x3Dcam = sRcw * (cv::Mat)points[index[index2]] + tcw;

                            if(x3Dcam.at<float>(2) >= 0.0f)
                            {
                                const cv::Point2f XY = mCurrentFrame.mpCamera->project(x3Dcam);

                                if((XY.x > mCurrentFrame.mnMinX) && (XY.x < (mCurrentFrame.mnMinX + StepX)))
                                {
                                    if((XY.y > mCurrentFrame.mnMinY) && (XY.y < (mCurrentFrame.mnMinY + StepY)))
                                        gridMatrix.at<float>(0, 0) = gridMatrix.at<float>(0, 0) + 1;

                                    if((XY.y > (mCurrentFrame.mnMinY + StepY)) && (XY.y < (mCurrentFrame.mnMinY + 2 * StepY)))
                                        gridMatrix.at<float>(1, 0) = gridMatrix.at<float>(1, 0) + 1;

                                    if((XY.y > (mCurrentFrame.mnMinY + 2 * StepY)) && (XY.y < (mCurrentFrame.mnMaxY)))
                                        gridMatrix.at<float>(2, 0) = gridMatrix.at<float>(2, 0) + 1;
                                }

                                if((XY.x > (mCurrentFrame.mnMinX + StepX)) && (XY.x < (mCurrentFrame.mnMinX + 2 * StepX)))
                                {
                                    if((XY.y > mCurrentFrame.mnMinY) && (XY.y < (mCurrentFrame.mnMinY + StepY)))
                                        gridMatrix.at<float>(0, 1) = gridMatrix.at<float>(0, 1) + 1;

                                    if((XY.y > (mCurrentFrame.mnMinY + StepY)) && (XY.y < (mCurrentFrame.mnMinY + 2 * StepY)))
                                        gridMatrix.at<float>(1, 1) = gridMatrix.at<float>(1, 1) + 1;

                                    if((XY.y > (mCurrentFrame.mnMinY + 2 * StepY)) && (XY.y < (mCurrentFrame.mnMaxY)))
                                        gridMatrix.at<float>(2, 1) = gridMatrix.at<float>(2, 1) + 1;
                                }

                                if((XY.x > (mCurrentFrame.mnMinX + 2 * StepX)) && (XY.x < (mCurrentFrame.mnMaxX)))
                                {
                                    if((XY.y > mCurrentFrame.mnMinY) && (XY.y < (mCurrentFrame.mnMinY + StepY)))
                                        gridMatrix.at<float>(0, 2) = gridMatrix.at<float>(0, 2) + 1;

                                    if((XY.y > (mCurrentFrame.mnMinY + StepY)) && (XY.y < (mCurrentFrame.mnMinY + 2 * StepY)))
                                        gridMatrix.at<float>(1, 2) = gridMatrix.at<float>(1, 2) + 1;

                                    if((XY.y > (mCurrentFrame.mnMinY + 2 * StepY)) && (XY.y < (mCurrentFrame.mnMaxY)))
                                        gridMatrix.at<float>(2, 2) = gridMatrix.at<float>(2, 2) + 1;
                                }
                    ...
                    if(abs(Angles[index[index2]] - LastKeyFrameAngles[i]) > geometryAngle)
                    {
                        meanAngles += abs(Angles[index[index2]] - LastKeyFrameAngles[i]);
                        ++nBigAngleChange;
                    }
                }
            }
        ...
        if(nDifferents == 0 || nMatchesLastKFandCurrentF == 0)
            return false;

        float maxval = *max_element(gridMatrix.begin<float>(), gridMatrix.end<float>());
        auto loc = find(gridMatrix.begin<float>(), gridMatrix.end<float>(), maxval);
        int idx = loc - gridMatrix.begin<float>();

        gridMatrix.at<float>(idx) = -10;

        float maxval2 = *max_element(gridMatrix.begin<float>(), gridMatrix.end<float>());
        auto loc2 = find(gridMatrix.begin<float>(), gridMatrix.end<float>(), maxval2);
        int secondidx = loc2 - gridMatrix.begin<float>();
        gridMatrix.at<float>(idx) = maxval;

        float DistBetw2Max = sqrt(pow((int(idx/3) - int(secondidx/3)), 2) + pow((int(idx%3) - int(secondidx%3)), 2));
        float criteriaForGridM = (DistBetw2Max * maxval2) / (2 * sqrt(2) * maxval);

        float centroidRow = ( sum(gridMatrix.row(2))[0] - sum(gridMatrix.row(0))[0] ) / (sum(gridMatrix)[0]);
        float centroidCol = ( sum(gridMatrix.col(2))[0] - sum(gridMatrix.col(0))[0] ) / (sum(gridMatrix)[0]);
        float centroidVec = sqrt(pow(centroidRow, 2) + pow(centroidCol, 2));

        if((mCurrentFrame.mnId - mnLastKeyFrameId) > 4)
        {
            criteriaForGridM = (DistBetw2Max * maxval2) / (sqrt(2) * maxval);
        }

        meanAngles /= (float)nBigAngleChange;

        if((mCurrentFrame.mnId - mnLastKeyFrameId) == 1)
        {
            matchr = nMatchesLastKFandCurrentF;
            diffr = nDifferents;
            CFr = points.size();
        }

        float sim1 = diffr*(float)nMatchesLastKFandCurrentF/matchr;
        float sim2 = diffr*(float)points.size()/(float)CFr;

        float similarization = (sim1 + sim2)/2;
        //float similarization = sim1;

        float gamma = 0;
        float teta = (matchr - (float)nMatchesLastKFandCurrentF)/matchr;
        float beta = ((float)nDifferents/(float)nMatchesLastKFandCurrentF - diffr/matchr);
        float alpha = 0;
        alpha = (float)nDifferents/(float)nMatchesLastKFandCurrentF - 0.5;

        if(nMatchesLastKFandCurrentF < matchr/2)
        {
            gamma = (matchr - 2*nMatchesLastKFandCurrentF)/(matchr + 2*nMatchesLastKFandCurrentF);
        }

        float coef = 1;
        if((mCurrentFrame.mnId - mnLastKeyFrameId) > 1)
        {
            //coef = pow((6 - (float)(mCurrentFrame.mnId - mnLastKeyFrameId)), 2)/(3 * (float)(mCurrentFrame.mnId - mnLastKeyFrameId));
            coef = (6 - (float)(mCurrentFrame.mnId - mnLastKeyFrameId))/3;
        }

        coef = 1;
        float thresh = (coef*teta - beta - alpha - gamma)*similarization + similarization;

        if(teta < 0.1)
        {
            thresh = 0.1*similarization + similarization;
        }

        /*static ofstream outputfile("diff.txt");
        outputfile<<mCurrentFrame.mnId<<"\t";
        outputfile<<mnLastKeyFrameId<<"\t";
        outputfile<<points.size()<<"\t";
        outputfile<<mnLastKeyFrameWorldPoints.size()<<"\t";
        outputfile<<nMatchesLastKFandCur

    VisibilityMatrixCondition();
}
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mCurrentFrame.mvpMapPoints.begin(), vend=mCurrentFrame.mvpMapPoints.end(); vit!=vend; vit++)
    {
        MapPoint* pMP = *vit;
        if(pMP)rentF<<"\t";
        outputfile<<nDifferents<<"\t";
        outputfile<<thresh<<"\t";
        int justForCheck = 0;
        if(criteriaForGridM > centroidVec)
            justForCheck = 1;
        outputfile<<justForCheck<<"\t";
        float a = 100*(float)nDifferents/(float)nMatchesLastKFandCurrentF;
        outputfile<<a<<"\t";
        cv::Mat deltaavgA = mCurrentFrame.mpImuPreintegratedFrame->avgA-mLastFrame.mpImuPreintegratedFrame->avgA;
        float nDelAvg = sqrt(pow(deltaavgA.at<float>(0),2) + pow(deltaavgA.at<float>(1),2) + pow(deltaavgA.at<float>(2),2));
        outputfile<<nDelAvg<<"\n";*/

        /*if((nMatchesLastKFandCurrentF == 0) && ((mCurrentFrame.mnId - mnLastKeyFrameId) > 1))
        {
            return true;
        }*/

        //if(nDifferents >= round(thresh))
        //if( (nDifferents > thresh) && ((mCurrentFrame.mnId - mnLastKeyFrameId) > 1) && ((float)nBigAngleChange/(float)nMatchesLastKFandCurrentF) > 0.35)
        if( (nDifferents > thresh) && ((mCurrentFrame.mnId - mnLastKeyFrameId) > 1) && (criteriaForGridM > centroidVec))
        {
            return true;
        }
        else
